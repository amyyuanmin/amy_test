    ...
    for i in range(0, int(loop)//(len(io_size_list)*len(q_depth_list)) + 1): 
        for io_size in io_size_list: # 4k, 32k, 128k, not consider multi-chunk as aligned with fw
            for q_depth in q_depth_list:
                ...
                with namespace.ioworker(io_size=io_size,
                            lba_random=False,
                            read_percentage=0,
                    ...
                    with namespace.ioworker(io_size=io_size,
                        ...
                        logging.info('Step 3. Issue PERST after wait {}s'.format(delay))
                        ...
                        if flag % 2 == 0:
                            ...
                            logging.info("Glitch: 120ms")
                            glitch = "120ms"
                        quarch_perst(delay, device, glitch)
                        pcie.reset(rst_fn = rst_fn)
                        controller.reset()            
                        controller.getfeatures(7).waitdone()  # an admin cmd after reset            
