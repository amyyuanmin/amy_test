...
def test_controller_reset_during_io(loop):
    reset_during_io(pcie, controller, namespace, subsystem, reset_util.controller_reset, loop)

def test_subsystem_reset_during_io(loop):
    reset_during_io(pcie, controller, namespace, subsystem, reset_util.subsystem_reset, loop)

def test_pcie_hot_reset_during_io(loop):
    reset_during_io(pcie, controller, namespace, subsystem, reset_util.pcie_hot_reset, loop)

def test_flr_during_io(loop):
    reset_during_io(pcie, controller, namespace, subsystem, reset_util.flr, loop)

def reset_during_io(pcie, ctrl, ns, subs, reset_fun, loop): 
    '''
    ...
    for i in range(0, int(loop)//(len(io_size_list)*len(q_depth_list)) + 1): 
        for io_size in io_size_list: # 4k, 32k, 128k, not consider multi-chunk as aligned with fw
            for q_depth in q_depth_list:
                ...
                logging.info("Step 2. Issue Mixed IO, Read 10G + Write 30G, io_size={}, q_depth={}".format(io_size * lba_size, q_depth))
                ...
                with ns.ioworker(io_size=io_size,
                    ...
                    with ns.ioworker(io_size=io_size,
                                ...
                                output_cmdlog_list=cmdlog_list):
                        time.sleep(delay)
                        logging.info('Step 3. Issue reset after wait {}s'.format(delay))
