...
def pytest_addoption(parser):
    ...
    parser.addoption("--cfg", action="store", default='fs_config.txt', help="cfg file")
    parser.addoption("--device_type", default='nvme', dest="device_type", help="device type, sata or nvme")
    parser.addoption("--ip", action="store", default="0.0.0.0", help="PI ip address")
...
def pi_ip(request):
    ver = request.config.getoption('ip')
    return ver