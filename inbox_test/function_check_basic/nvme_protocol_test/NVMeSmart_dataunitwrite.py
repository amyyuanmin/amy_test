...
def verify_data_unit_write_test(ns_data, ctrl, ns, result_file, loop):
    results_dict = {}
    finalResult = []
...
#step3: Interating  over data size list 1 by 1
        before_test_data=int(smart.all_smart_data("data_units_written"))
        total_increment=0
        for index in data_size:
            increment_value=(loop*index//512)//1000
            total_increment+=increment_value
            current_value=int(smart.all_smart_data("data_units_written"))
...
#step4: Iterating complete 1000 loop for every lbads
            ...
            if flag==False:
                ...
                finalResult.append(0)
            After_Write_Value=int(smart.all_smart_data("data_units_written"))
        ...
        finalResult.append(1)
    with open("{}/dataunitwrite.log".format(result_file), 'w+') as f:
        ...
        logging.info('\n')
        if 1 in finalResult:
            logging.error("TC-1 IOL (Test 1.3 : Case 8) - Data Units Written : Failed")
            f.write("\nTC-1 IOL (Test 1.3 : Case 8) - Data Units Written: FAILED\n")
            result = "FAILED"
            #return -1
        else:
            logging.info("TC-1 IOL (Test 1.3 : Case 8) - Data Units Written : PASSED")
            f.write("\nTC-1 IOL (Test 1.3 : Case 8) - Data Units Written: PASSED\n")
            result = "PASSED"
        f.close()
        return result
...
def main(ctrl, ns, result_file):
    ...
    logging.info("\n ")
    logging.info("###########################################################################")
    logging.info("#       TC-1 IOL (Test 1.3 : Case 8) - Data Units Written                 #")
    logging.info("###########################################################################")
    loop=1000
    logging.info("No. of iteration to  perform write  operation: {}".format(loop))
    return verify_data_unit_write_test(ns_data, ctrl, ns, result_file, loop)