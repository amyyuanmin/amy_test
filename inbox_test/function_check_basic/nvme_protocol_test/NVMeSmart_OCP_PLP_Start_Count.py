...
def NVMeSmart_OCP_PLP_Start_Count(ns_data, ctrl, ns, result_file):
    result = "FAILED"
    ...
    try:
        ...
        # PLP_Start_Count   FW doesn't support Power loss until now (2021.11.15)
    ...
    with open("{}/OCP_Smart_result.log".format(result_file), 'a+') as f:
        f.write("NVMeSmart_OCP_PLP_Start_Count      {} \n".format(result))
        f.close()
    return result
...
def main(ctrl, ns, result_file):
    ...
    logging.info("\n ")
    logging.info("###########################################################################")
    logging.info("#                    NVMeSmart_OCP_PLP_Start_Count                        #")
    logging.info("###########################################################################")

    return NVMeSmart_OCP_PLP_Start_Count(ns_data, ctrl, ns, result_file)