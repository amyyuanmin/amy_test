...
def NVMeSmart_Thm_Temp2_Trans_Count(ctrl, ns, result_file):
    result = "FAILED"
    smart = SMART(ctrl, ns)
    ...
    try:
        ...
        if HCTMA == 0:
            ...
            cnt_1=int(smart.all_smart_data("thm_temp2_trans_count"))
            logging.info("Thermal Management Temperature 2 Transition Count  =   {}".format(cnt_1))
            power_state_1 = int(ns.get_power_mgmt())
            logging.info("Power State Current  =   {}".format(power_state_1))
            # Get default Thermal Management Temperature 2
            ...
            TMT2 = temp_1 -2
            TMT = TMT2+(TMT1<<16)
            logging.info("set TMT2: {}; SET tmt1: {}".format(TMT2,TMT1))
            ...
            if power_state_2>power_state_1:
                # Get the Thm_Temp1_Trans_Count from Smart
                cnt_2=int(smart.all_smart_data("thm_temp2_trans_count"))
                logging.info("Thermal Management Temperature 2 Transition Count  =   {}".format(cnt_2))
                if cnt_2 == cnt_1 +1:
                    logging.info("Check Thermal Management Temperature 2 Transition Count Pass")
                    result = "Pass"
                else:
                    logging.error("Check Thermal Management Temperature 2 Transition Count Fail")
            else:
            ...
            logging.info("Thermal Management Temperature 1 Current: {}".format(TMT_get&0xffff))
            if TMT_get != TMT_default:
                logging.error("Set feature to default fail")
                result = "Fail"
    ...
    with open("{}/NVMe_Smart_result.log".format(result_file), 'a+') as f:
        f.write("NVMeSmart_Thm_Temp2_Trans_Count             {} \n".format(result))
        f.close()
    return result
...
def main(ctrl, ns, result_file):
    logging.info("\n ")
    logging.info("###########################################################################")
    logging.info("#         NVMe_Thermal_Management_Temperature_2_Transition_Coun           #")
    logging.info("###########################################################################")

    return NVMeSmart_Thm_Temp2_Trans_Count(ctrl, ns, result_file)