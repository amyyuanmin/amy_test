...
def NVMeSmart_Thm_Temp1_Total_Time(ctrl, ns, result_file):
    result = "FAILED"
    smart = SMART(ctrl, ns)
    ...
    try:
        ...
        if HCTMA == 0:
            ...
            time_1=int(smart.all_smart_data("thm_temp1_total_time"))
            logging.info("Total Time For Thermal Management Temperature 1  =   {}".format(time_1))
            ...
            if power_state_2>power_state_1:
                ...
                time_2=int(smart.all_smart_data("thm_temp1_total_time"))
                logging.info("Total Time For Thermal Management Temperature 1  =   {}".format(time_2))
                if time_2 > time_1:
                    logging.info("Check Total Time For Thermal Management Temperature 1 Pass")
                    result = "Pass"
                else:
                    logging.error("Check  Total Time For Thermal Management Temperature 1 Fail")
            else:
    ...
    with open("{}/NVMe_Smart_result.log".format(result_file), 'a+') as f:
        f.write("NVMeSmart_Thm_Temp1_Total_Time             {} \n".format(result))
        f.close()
    return result
...
def main(ctrl, ns, result_file):
    logging.info("\n ")
    logging.info("###########################################################################")
    logging.info("#          NVMe_Total_Time_For_Thermal_Management_Temperature 1           #")
    logging.info("###########################################################################")

    return NVMeSmart_Thm_Temp1_Total_Time(ctrl, ns, result_file)