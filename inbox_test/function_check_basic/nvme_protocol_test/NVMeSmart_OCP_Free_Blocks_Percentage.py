...
def NVMeSmart_OCP_Free_Blocks_Percentage(ns_data, ctrl, ns, result_file):
    result = "FAILED"
    ...
    with open("{}/OCP_Smart_result.log".format(result_file), 'a+') as f:
        f.write("NVMeSmart_OCP_Free_Blocks_Percentage     {} \n".format(result))
        f.close()
    return result
...
def main(ctrl, ns, result_file):
    ...
    logging.info("\n ")
    logging.info("###########################################################################")
    logging.info("#                 NVMeSmart_OCP_Free_Blocks_Percentage                    #")
    logging.info("###########################################################################")

    return NVMeSmart_OCP_Free_Blocks_Percentage(ns_data, ctrl, ns, result_file)