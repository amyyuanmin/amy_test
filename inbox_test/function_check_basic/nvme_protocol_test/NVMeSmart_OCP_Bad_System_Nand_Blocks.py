...
def NVMeSmart_OCP_Bad_System_Nand_Blocks(ns_data, ctrl, ns, result_file):
    result = "FAILED"
    smart = SMART(ctrl, ns)
    try:
    ...
    with open("{}/OCP_Smart_result.log".format(result_file), 'a+') as f:
        f.write("NVMeSmart_OCP_Bad_System_Nand_Blocks         {} \n".format(result))
        f.close()
    return result
...
def main(ctrl, ns, result_file):
    ...
    logging.info("\n ")
    logging.info("###########################################################################")
    logging.info("#                         NVMeSmart_OCP_Bad_System_Nand_Blocks                        #")
    logging.info("###########################################################################")

    return NVMeSmart_OCP_Bad_System_Nand_Blocks(ns_data, ctrl, ns, result_file)