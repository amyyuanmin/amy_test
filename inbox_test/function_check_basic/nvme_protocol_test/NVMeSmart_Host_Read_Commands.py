...
def verify_host_read_commands(ns_data, ctrl, ns, result_file, loop):
    results_dict = {}
    finalResult = []
    ...
    try:
        ...
        before_test_data=int(smart.all_smart_data('host_reads'))
        total_increment=0
        for index in data_size:
            increment_value=loop
            total_increment+=increment_value
            current_value=int(smart.all_smart_data("host_reads"))
            ...
            if flag==False:
                ...
                finalResult.append(0)
            After_Read_Value=int(smart.all_smart_data("host_reads"))
        ...
        finalResult.append(1)
    with open("{}/Host_Read_Commands.log".format(result_file), 'w+') as f:
        ...
        logging.info('\n')
        if 1 in finalResult:
            logging.error("TC-3  SMART_LOG: Host Read Commands : Failed")
            f.write("\nTC-3  SMART_LOG: Host Read Commands: FAILED\n")
            logging.error("test_case Failed")
            result = "FAILED"
            #return -1
        else:
            logging.info("TC-3  SMART_LOG: Host Read Commands : PASSED")
...
def main(ctrl, ns, result_file):
    ...
    logging.info("\n ")
    logging.info("###############################################################")
    logging.info("#              TC-3  SMART_LOG: Host Read Commands           #")
    logging.info("###############################################################")
    loop=1000
    logging.info("No. of iteration to  perform read  operation: {}".format(loop))
    return verify_host_read_commands(ns_data, ctrl, ns, result_file, loop)